...
class ModalTransitionAnimator: BaseTransitionAnimator {
    
    override func animateTransition(transitionContext: UIViewControllerContextTransitioning) {
        // Here, we perform the animations necessary for the transition to bring on the main menu
        ...
        let otherVC = isPresentation ? fromVC : toVC
        ...
        UIView.animateWithDuration(duration, delay: 0, usingSpringWithDamping: 300.0, initialSpringVelocity: 5.0, options: .AllowUserInteraction | .BeginFromCurrentState, animations: { () -> Void in
            ...
            if isPresentation {
                otherView.transform = CGAffineTransformMakeScale(self.finalScale, self.finalScale)
            } else {
                otherView.transform = CGAffineTransformIdentity
            }
            }) { (finished) -> Void in
...
class FromMainMenuTransitionAnimator: BaseTransitionAnimator {
